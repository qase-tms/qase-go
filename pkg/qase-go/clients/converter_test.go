...
func TestV2Converter_ConvertTestResult_TestopsIDTypes(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			testopsID: "invalid",
			expectErr: false, // Now we log warning instead of failing
			checkFunc: func(result *api_v2_client.ResultCreate) bool {
				// Should not have TestopsID set
				return result.GetTestopsId() == 0 && len(result.GetTestopsIds()) == 0
			},
		},
		{
...
func TestV2Converter_ConvertTestResult_HandlesConversionErrorsGracefully(t *testing.T) {
	converter := NewV2Converter()

	// Create a test result with problematic data that would normally cause conversion to fail
	domainResult := domain.NewTestResult("Test With Issues")
	domainResult.Execution.Status = domain.StatusPassed

	// Add a problematic attachment (this will cause an error but shouldn't fail the entire result)
	problematicAttachment := domain.Attachment{
		FileName: "test.txt",
		Content:  []byte("test content"),
		// No ID and no file path - this will cause issues but shouldn't fail the result
	}
	domainResult.Attachments = []domain.Attachment{problematicAttachment}

	// Convert the result - it should succeed despite attachment issues
	apiResult, err := converter.ConvertTestResult(domainResult)

	// The result should still be converted successfully
	if err != nil {
		t.Fatalf("Expected successful conversion despite attachment issues, got error: %v", err)
	}

	if apiResult == nil {
		t.Fatal("Expected non-nil result despite attachment issues")
	}

	// Verify basic fields are still present
	if apiResult.GetTitle() != "Test With Issues" {
		t.Errorf("Expected title 'Test With Issues', got: %s", apiResult.GetTitle())
	}
	...
	// Attachments should be empty due to processing issues
	if apiResult.GetAttachments() != nil && len(apiResult.GetAttachments()) > 0 {
		t.Errorf("Expected no attachments due to processing issues, got: %v", apiResult.GetAttachments())
	}
}

func TestV2Converter_ConvertTestResult_HandlesStepConversionErrorsGracefully(t *testing.T) {
	converter := NewV2Converter()

	// Create a test result with problematic steps
	domainResult := domain.NewTestResult("Test With Problematic Steps")
	domainResult.Execution.Status = domain.StatusPassed

	// Add a step with problematic data
	problematicStep := domain.TestStep{
		Data: domain.StepTextData{
			Action: "Problematic Step",
		},
		Execution: domain.StepExecution{
			Status: domain.StepStatusPassed,
		},
		// Add problematic attachment to step
		Attachments: []domain.Attachment{
			{
				FileName: "step-attachment.txt",
	...
	domainResult.Steps = []domain.TestStep{problematicStep}

	// Convert the result - it should succeed despite step issues
	apiResult, err := converter.ConvertTestResult(domainResult)

	// The result should still be converted successfully
	if err != nil {
		t.Fatalf("Expected successful conversion despite step issues, got error: %v", err)
	}

	if apiResult == nil {
		t.Fatal("Expected non-nil result despite step issues")
	}

	// Verify basic fields are still present
	if apiResult.GetTitle() != "Test With Problematic Steps" {
		t.Errorf("Expected title 'Test With Problematic Steps', got: %s", apiResult.GetTitle())
	}

	// Steps should still be present despite attachment issues
	if apiResult.GetSteps() == nil || len(apiResult.GetSteps()) == 0 {
		t.Error("Expected steps to be present despite attachment issues")
	}

	// Verify the step was converted successfully
	step := apiResult.GetSteps()[0]
	stepData := step.GetData()
	if stepData.GetAction() != "Problematic Step" {
		t.Errorf("Expected step action 'Problematic Step', got: %s", stepData.GetAction())
	}

	// Step attachments should be empty due to processing issues
	if stepData.GetAttachments() != nil && len(stepData.GetAttachments()) > 0 {
		t.Errorf("Expected no step attachments due to processing issues, got: %v", stepData.GetAttachments())
	}
}
